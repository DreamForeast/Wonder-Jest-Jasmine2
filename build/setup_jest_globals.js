...
var _jestSnapshot = require('wonder-jest-snapshot');
 ...
 *
 * 
 */const addSuppressedErrors = result => {var _getState = (0, _expect.getState)();const suppressedErrors = _getState.suppressedErrors;(0, _expect.setState)({ suppressedErrors: [] });if (suppressedErrors.length) {result.status = 'failed';result.failedExpectations = suppressedErrors.map(error => ({ actual: '',
...
const addAssertionErrors = result => {
  ...
  if (assertionErrors.length) {
    const jasmineErrors = assertionErrors.map((_ref) => {let actual = _ref.actual,error = _ref.error,expected = _ref.expected;
      return {
        actual,
        expected,
        message: error.stack,
...
{let config = _ref2.config,globalConfig = _ref2.globalConfig,localRequire = _ref2.localRequire,testPath = _ref2.testPath;
